...
const props = defineProps(['itemData', 'methodData']);
const unit = ref(
  props.methodData.unit !== null && props.methodData.unit !== '' ? true : false
);
const inspection = ref(new inspectionHelper());
inspection.value.setId(props.methodData.logic);
inspection.value.standards = JSON.parse(props.methodData.standard);
const emits = defineEmits(['submitted']);
const formData = reactive({
  label: props.methodData.label,
  logic: props.methodData.logic,
  unit: props.methodData.unit,
});
...
const submit = async () => {
  try {
    ...
    await store.fetchData(
      reqData,
      `/inspections/methods/${props.methodData.methodId}`,
      'PUT'
    );
    emits('submitted');
  } catch (error) {
